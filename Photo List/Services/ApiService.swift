...
class ApiService: ApiServiceProtocol {
    ...
    func fetchPhotos(completed: @escaping (Result<[Photo], Error>) -> Void) {
        AF.request(apiBaseUrl + "/photos").responseDecodable(of: [Photo].self) { response in
    ...
    func fetchComments(forPhotoId photoId: Int, completed: @escaping (Result<[Comment], Error>) -> Void) {
        AF.request(apiBaseUrl + "/photos/" + String(photoId) + "/comments").responseDecodable(of: [Comment].self) { response in
